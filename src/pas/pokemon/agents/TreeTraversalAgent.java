 ...
 * and move-ordering that considers "power * type effectiveness"
 * vs. the opponent's active Pokemon, which may have up to 2 types.
 */
public class TreeTraversalAgent
    extends Agent
{
    private static final int MAX_DEPTH = 3;   // or 4, as you prefer
    private long maxThinkingTimePerMoveInMS;

    public TreeTraversalAgent()
    {
        super();
        this.maxThinkingTimePerMoveInMS = 60000; // e.g., 60s
    }

    // We do not override any method from the supertype "Agent" named getMaxThinkingTimePerMoveInMS(),
    // so we omit @Override to prevent compile errors
        ...
        implements Callable<Pair<MoveView, Long>>
	{
        ...
        {
            // pass in the static MAX_DEPTH from our agent
        ...
        public MoveView stochasticTreeSearch(BattleView rootView) {
            ...
            // Sort + limit moves
            myMoves = orderAndLimitMoves(rootView, this.getMyTeamIdx(), oppIdx, myMoves);
    ...
    public Integer chooseNextPokemon(BattleView state)
    {
        ...
        // We'll do a smaller depth for nextPokemon
        int shallowDepth = 1;
        for (SwitchMoveView sw : switchMoves) {
            ...
            // Build a small tree
            MinimaxTreeBuilder tb = new MinimaxTreeBuilder(outState, shallowDepth, myTeamIdx){};
...
// No quiescence, but includes "power * type effectiveness"
// with up to 2 types from getCurrentType1() / getCurrentType2().
// =================================================================
abstract class MinimaxTreeBuilder
{
    public static final int MAX_MOVES_TO_EXPAND = 5; // Make it public & static
    ...
    /**
     * Returns "effective power" = basePower * product_of_typeMods
     * If switch or no base power => 0
     * Accounts for up to two types:
     *   foePoke.getCurrentType1()
     *   foePoke.getCurrentType2()
     */
...
//         int oppTeamIdx = oppTeam.getBattleIdx();
//         int myTeamIdx = myTeam.getBattleIdx();
//         int exploreDepth = computeDynamicDepth(state) - 1;
